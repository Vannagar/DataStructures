...
public class PostfixEvaluator 
{
	...
	public static String[] tokenize(String exp)
	{
		...
		{
			tokensarr[tokens.size()-x-1]=tokens.get(x);
		}
		return tokensarr;
	}
	...
	public static double operate(Tree connections)
	{
		...
		{
			return operate(connections.getLeaves().get(1))-operate(connections.getLeaves().get(0));
		}
		else if(connections.getValue().equals("*"))
		{
			return operate(connections.getLeaves().get(0))*operate(connections.getLeaves().get(1));
		...
		{
			return Math.pow(operate(connections.getLeaves().get(1)),operate(connections.getLeaves().get(0)));
		}
	}
	public class Tree
	{
